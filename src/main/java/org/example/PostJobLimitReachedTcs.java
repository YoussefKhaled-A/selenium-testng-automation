...
public class PostJobLimitReachedTcs {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        // -------------------- POST JOB PAGE (TC5_LimitReached) --------------------
        ...
        // Click suggested title if visible (like Katalon)
        try {
...
// Main Work Field (select by sending text then ENTER)
        ...
        // Salary: min 100, max 300  (from TC5)
        ...
        // -------------------- RESULT CHECK (NEGATIVE – LIMIT REACHED) --------------------
        // Precondition: account already reached max active jobs.
        // Expectation: system should NOT fully post this job → still on post page / show message.
        String url = driver.getCurrentUrl();

        if (url.contains("/employer/job/post")) {
            System.out.println("✅ TC5 Passed: Job limit reached – new job not activated (still on post page).");
        } else {
            System.out.println("❌ TC5 Failed: Job seems posted despite supposed limit.");
        }
